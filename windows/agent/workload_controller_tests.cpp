...
int main(int argc,char** argv){try{
 ...
 auto wait=[&]{for(int i=0;i<300;i++){const auto value=controller.snapshot();if(!value["recordingId"].is_null())return value;Sleep(10);}throw std::runtime_error("Controller completion deadline: "+controller.snapshot().dump());};
