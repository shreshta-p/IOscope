...
inline void validate_recording_links(const RecordJson& recording,const RecordJson& mapping){
 ...
 // .get<std::string>() on the RHS (not needed on the Windows/MSVC build): nlohmann-json
 // 3.12's C++20 operator==(ScalarType) is constrained to is_scalar_v, which excludes
 // std::string, so std::string==json doesn't resolve under GCC/libstdc++ the way it
 // does under MSVC's STL. Same comparison, just spelled to avoid that overload gap.
 recording_require(terminal.contains(metadata["outcome"].get<std::string>())?previousState==metadata["outcome"].get<std::string>():!terminal.contains(previousState),"Terminal outcome mismatch");
