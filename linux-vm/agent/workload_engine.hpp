...
public:
 ...
 void ready()override{
  ...
  if(std::filesystem::exists(root)){reject_symlink_ancestors(root);if(std::filesystem::directory_iterator(root)!=std::filesystem::directory_iterator())throw std::runtime_error("Owned scratch contains an unfinished run; inspect recovery before starting another workload");}
 }
 nlohmann::json metadata(const nlohmann::json& workload,const std::string& id)override{
  auto argv=nlohmann::json::array();for(const auto& arg:fio_arguments(workload,local_directory()/"scratch"/id/"data.bin"))argv.push_back(arg);
 ...
 EngineResult execute(const nlohmann::json& workload,const std::string& id,const std::atomic<bool>& cancel,
  ...
  try{
   ...
   const auto prepared=run_child_process(id+"-prepare",preparation.string(),{id,std::to_string(bytes)},cancel,std::chrono::seconds(bytes/(32*MiB)+15),watchdog,scratch->preparation_fd());
