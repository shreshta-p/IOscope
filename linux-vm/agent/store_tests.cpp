...
char executable[PATH_MAX+1]{};const auto length=readlink("/proc/self/exe",executable,PATH_MAX);if(length<=0)throw std::runtime_error("Test executable unavailable");
const auto crashPath=directory/"crash.sqlite";
const pid_t pid=fork();if(pid<0)throw std::runtime_error("Crash fixture failed to fork");
if(pid==0){execl(executable,executable,"--crash",crashPath.c_str(),static_cast<char*>(nullptr));_exit(127);}
int status=0;const auto waited=waitpid(pid,&status,0);
if(waited!=pid||!WIFEXITED(status)||WEXITSTATUS(status)!=99)throw std::runtime_error("Crash fixture failed");
