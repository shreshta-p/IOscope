...
export function Runs({ current, onReplay, onExport, onImport, onNotice }: Props) {
  ...
  async function artifact(id: string) {
    try {
      ...
      const extension = value.kind === 'diskspd-xml' ? '.xml' : value.kind === 'fio-json' ? '.json' : '.txt';
      const mimeType =
        value.kind === 'diskspd-xml'
          ? 'application/xml'
          : value.kind === 'fio-json'
            ? 'application/json'
            : 'text/plain';
      const url = URL.createObjectURL(new Blob([value.payload], { type: mimeType }));
